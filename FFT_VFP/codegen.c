...
#define MAP_ANONYMOUS 0x20	
	...
	*fp = 0xed2d8b10; fp++;
	...
	ADDI(&fp, 6, 4, 2*N);
	ADDI(&fp, 9, 8, 2*N);
	*fp = LDRI(12, 0, ((uint32_t)&p->offsets) - ((uint32_t)p)); fp++;
	ADDI(&fp, 1, 0, 0);
	ADDI(&fp, 0, 2, 0),
#endif
	...
	*fp++ = 0x07;
	...
	while(pps[0]) {
      ...
      CALL(&fp, x_4_addr);
		...
		if(pps[0] > leafN) 
			pN = pps[0];
		pLUT = p->ws_is[__builtin_ctzl(pps[0]/leafN)-1]*8;
		count += 4;
		pps += 2;
	}
#endif
...
#endif
  *fp = LDRI(2, 1, ((uint32_t)&p->ws) - ((uint32_t)p)); fp++;
	MOVI(&fp, 1, 0);
	int32_t pAddr = 0;
	int32_t pN = 0;
	int32_t pLUT = 0;
